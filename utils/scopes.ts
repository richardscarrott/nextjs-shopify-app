// https://github.com/Shopify/shopify-api-node/blob/1bb138f95d166e2602f6d470a887194f8988c430/src/auth/scopes/index.ts#L69
...
export class AuthScopes {
  ...
  constructor(scopes: string | string[] | undefined) {
    ...
    this.compressedScopes = new Set(
      Array.from(scopeSet).filter((x) => !impliedSet.has(x))
    );
    this.expandedScopes = new Set();
    scopeSet.forEach((x) => this.expandedScopes.add(x));
    impliedSet.forEach((x) => this.expandedScopes.add(x));
  }
  ...
  public toArray() {
    return Array.from(this.compressedScopes);
  }
